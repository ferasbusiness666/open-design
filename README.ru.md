...
Ярлык Windows: вы также можете дважды щёлкнуть `OpenDesign.exe` в корне репозитория. Он выполнит `pnpm install`, если зависимостей не хватает, а затем запустит Open Design через `pnpm tools-dev`.
